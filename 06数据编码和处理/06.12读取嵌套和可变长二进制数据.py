...
print(phead.min_x, phead.max_x)
print(phead.min_y, phead.max_y)
print(phead.num_polys)


# 这个很有趣，不过这种方式还是有一些烦人的地方。
# 首先，尽管你获得了一个类接口的便利， 但是这个代码还是有点臃肿，
# 还需要使用者指定很多底层的细节(比如重复使用 StructField ，指定偏移量等)。
...
print(phead.file_code == 0x1234)
print(phead.min_x, phead.min_y)
print(phead.max_x, phead.max_y)
print(phead.num_polys)
...
phead = PolyHeader.from_file(f)
print(phead.file_code == 0x1234)
print(phead.min.x, phead.min.y)
print(phead.max.x, phead.max.y)
print(phead.num_polys)
...
print(polydata)
# [<__main__.SizeRecord object at 0x0000021158EC07F0>,...
...
# 这么做可以帮助避免额外输入，但是在定义的中间我们仍然可能切换顺序的。 比如，你可能有一些比较复杂的结构，就像下面这样：
