...
print(data.split())
print(data.replace(b'Hello', b'Hello Lee'))
...
# 比如：
with open("test\中文文件名.txt", 'w+', encoding="utf-8") as f:
    f.write("test中文")

import os

print(os.listdir('.'))  # ['02.01使用多个界定符分割字符串.py',
print(os.listdir(b'.'))  # [b'02.01\xe4\xbd\xbf\xe7\x94\xa8\xe5..

# 注意例子中的最后部分给目录名传递一个字节字符串是怎样导致结果中文件名以未解码字节返回的。
# 在目录中的文件名包含原始的UTF-8编码。 参考5.15小节获取更多文件名相关的内容。
