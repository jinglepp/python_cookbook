...
print(unicodedata.normalize('NFD', s.translate(remap)))
# pýtĥöñ is awesome
print(unicodedata.normalize('NFD', s.translate(remap)).translate(cmb_chrs))
# python is awesome
...
# 580
# print(list(map(chr, (c for c in digitmap))))
...
# 一般来讲，代码越简单运行越快。 对于简单的替换操作， str.replace() 方法通常是最快的，甚至在你需要多次调用的时候。
# 比如，为了清理空白字符，你可以这样做：
def clean_spaces(s):
    s = s.replace('\r', '')
    s = s.replace('\t', ' ')
