...
# 因此，你无需担心对象记录的定义，它总是能工作。 举个例子，如果要处理多个对象，你可以这样做：
import pickle

with open('somedata', 'wb') as f:
    pickle.dump([1, 2, 3, 4], f)  # TypeError: an integer is required (got type _io.BufferedWriter)
    pickle.dump('hello', f)
...
# 如果你需要移动大量的数组数据，你最好是先在一个文件中将其保存为数组数据块或使用更高级的标准编码方式
# 如HDF5 (需要第三方库的支持)。
