...
def echo_client(client_sock, addr):
    ...
    client_in = open(client_sock.fileno(), 'rt', encoding='utf-8', closefd=False)
    client_out = open(client_sock.fileno(), 'wt', encoding='utf-8', closefd=False)
...
# import sys
#
# bstdout = open(sys.stdout.fineno(), 'wb', closefd=False)
# bstdout.write(b'Hello World\n')
# bstdout.flush()

# 尽管可以将一个已存在的文件描述符包装成一个正常的文件对象， 但是要注意的是并不是所有的文件模式都被支持，
# 并且某些类型的文件描述符可能会有副作用 (特别是涉及到错误处理、文件结尾条件等等的时候)。
