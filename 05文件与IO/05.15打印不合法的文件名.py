...
for name in files:
    print(name)
# UnicodeEncodeError:
# %由于首行加了#coding:utf-8,文件以utf-8保存,不会报此错误
# 程序崩溃的原因就是字符 \udce4是一个非法的Unicode字符。 它其实是一个被称为代理字符对的双字符组合的后半部分。
# 由于缺少了前半部分，因此它是个非法的Unicode。 所以，唯一能成功输出的方法就是当遇到不合法文件名时采取相应的补救措施。
# 比如可以将上述代码修改如下：
for name in files:
    try:
        print(name)
